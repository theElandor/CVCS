...
def validation_loss(net, Loader_validation, crit, device, batch_size, show_progress=False):
    loss_values = []
    net.eval()
    with torch.no_grad():
        for c in range(len(Loader_validation)):
...
def save_model(epoch, net, opt, scheduler, train_loss, val_loss, macro_precision, weighted_precision, conf_flat,
               conf_normalized, batch_size, checkpoint_dir, optimizer):
    torch.save({
        'epoch': epoch,
...
def inference(net, patch_size, dataset, indexes, device, converter, mask_only=False, border_correction=None):
    ...
    with torch.no_grad():
        for index in indexes:
            ...
            image, mask, context, padded_patch = image.to(device), mask.to(device), context.to(device), padded_patch.to(
                device)
...
def load_optimizer(config, net):
    ...
    if optimizer == 'SGD1':
        ...
        return torch.optim.Adam(net.parameters(), betas=(0.9, 0.999), weight_decay=0.1, lr=0.0008)
    else:
...
def plot_priors(confusion, sorted=True, path=None):
    ...
    colors = [(label_to_color[x[0]][0] / 255, label_to_color[x[0]][1] / 255, label_to_color[x[0]][2] / 255) for x in
              support]
