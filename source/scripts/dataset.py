...
def _get_cropped_data(image: torch.tensor, index_mask: torch.tensor, color_mask: torch.tensor, tly: int, tlx: int,
                      p: int):
...
class GID15(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        tif_img, mask_img, _ = _get_cropped_data(self.last_image, self.last_target, self.last_target, tly, tlx,
                                                 self.patch_shape[0])
...
class IterableChunk(torch.utils.data.IterableDataset):
    ...
    def load_images(self, names):
        ...
        index_masks = [
            tv_tensors.Mask(Image.open(os.path.join(self.indexdir, Path(name).stem + "_15label.png")))[0].unsqueeze(0)
            for name in names]
