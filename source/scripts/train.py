...
from utils import validation_loss, save_model, eval_model, load_network, load_gaofen, print_sizes
import yaml
import sys
inFile = sys.argv[1]
...
if not Path(config['checkpoint_directory']).is_dir():
    ...
    for epoch in range(last_epoch, config['epochs']):
        ...
        validation_loss_values += validation_loss(net, validation_loader, crit, device, show_progress=config['val_progress'])
        ...
        if (epoch+1) % config['freq'] == 0: # save checkpoint every freq epochs            
            save_model(epoch, net, opt, loss, training_loss_values, validation_loss_values, macro_precision, weighted_precision, config['batch_size'], config['checkpoint_directory'])
