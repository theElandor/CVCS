...
class UnetTorch(nn.Module):
    def __init__(self, device, in_channels=3, out_channels=16, init_features=224, pretrained=False):
        ...
        self.model = torch.hub.load('mateuszbuda/brain-segmentation-pytorch', 'unet', in_channels=in_channels,
                                    out_channels=out_channels, init_features=init_features, pretrained=pretrained).to(
            device)
        self.wrapper = True
        self.requires_context = False
        self.returns_logits = True
